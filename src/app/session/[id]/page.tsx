    ...
    const fetchSession = async () => {
      try {
        ...
        const { data, error } = await supabase
          ...
          .select(`
            *,
            profiles:counselor_id (full_name, avatar_url)
          `)
          .eq('id', id)
          .single();
    ...
    <div className="h-screen bg-[#F8FAFC] flex flex-col selection:bg-udanix-blue/10">
        ...
        <div className="flex items-center gap-8">
          ...
          <div className="flex flex-col">
            <h1 style={{ fontFamily: 'var(--font-space-grotesk)' }} className="text-xl font-black text-slate-900 leading-none uppercase tracking-tighter">
              Session <span className="text-udanix-blue">{session.topic || 'Untitled'}</span>
            </h1>
            <div className="flex items-center gap-2.5 mt-2">
              ...
              <span className="text-[9px] text-slate-400 font-black uppercase tracking-[0.2em] leading-none">Encrypted Neural Bridge Active</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-5">
          <div className="flex -space-x-3 mr-4">
            <div className="w-10 h-10 rounded-xl border-4 border-white bg-slate-900 flex items-center justify-center text-[10px] font-black text-white shadow-xl uppercase">
              {session.profiles?.full_name?.substring(0, 2) || 'EX'}
            </div>
      ...
      <div className="h-14 bg-slate-900 px-10 flex items-center justify-between shrink-0 text-white z-50">
        <div className="flex items-center gap-3">
          <ShieldCheck className="w-4 h-4 text-udanix-cyan" />
          <span className="text-[10px] font-black uppercase tracking-[0.25em] opacity-80">Quantum-Safe Encryption Matrix Engaged</span>
        </div>
        <div className="flex items-center gap-8">
          <div className="flex items-center gap-4">
            <span className="text-[10px] font-black uppercase tracking-widest opacity-60">Status: {session.status}</span>
            <div className="w-px h-3 bg-white/20" />
            <span className="text-[10px] font-black uppercase tracking-widest text-udanix-cyan">Neural Link Stabilized</span>
          </div>
        </div>
      </div>
    </div>
